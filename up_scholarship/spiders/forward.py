...
class ForwardAppSpider(scrapy.Spider):
	name = 'forward'
	...
	def __init__(self, *args, **kwargs):
		...
		super(ForwardAppSpider, self).__init__(*args, **kwargs)
	...
	def forward_app(self, response):
		""" Open the verify application page
			...
			response -- previous scrapy response.
		"""
		self.logger.info('In forward_app. Last URL: %s', response.url)
		student = self.students[self.i_students]
		if self.process_errors(response, TestStrings.error) or \
			...
			url = self.url_provider.get_institute_forward_url(student.get(FormKeys.std(), ''))
	...
	def forward(self, response):
		""" Select forward from drop down.
			...
			response -- previous scrapy response.
		"""
		self.logger.info('In forward. Last URL: %s', response.url)
		student = self.students[self.i_students]
	...
	def parse(self, response):
		...
		if not application_forwarded and self.process_errors(response, TestStrings.error, captcha_check=False):
			student = self.students[self.i_students]
			url = self.url_provider.get_institute_login_url(student.get(FormKeys.std(), ''))
			yield scrapy.Request(url=url, callback=self.get_district, dont_filter=True, errback=self.errback_next)
		else:
			...
			if application_forwarded:
				...
				student[FormKeys.status()] = message
				student[FormKeys.app_forwarded()] = "Y"
			else:
	...
	def skip_to_next_valid(self) -> int:
		...
		for x in range(self.i_students, length):
			...
			if student.get(FormKeys.skip(), '') == 'N' and reg_year == datetime.today().year and student.get(
				FormKeys.final_submitted(), '') == 'Y' and student.get(FormKeys.app_verified(), '') == 'Y'\
					and student.get(FormKeys.app_forwarded(), '') == 'N':
				return_index = x
		...
		if return_index != -1:
			...
			self.logger.info(
				'Forwarding application of: ' + student.get(FormKeys.name(), '') + ' of std: ' + student.get(
					FormKeys.std(), ''))
			return return_index
		else:
	...
	def process_errors(self, response, check_str, html=True, captcha_check=True):
		...
		if error:
			utl.save_file_with_name(student, response, WorkType.forward, str(datetime.today().year), is_debug=True)
